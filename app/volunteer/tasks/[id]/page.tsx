  ...
  const handleStatusUpdate = async () => {
    ...
    try {
      const isCompletedWithPayload =
        newStatus === "completed" &&
        (evidenceFiles.length > 0 || completionNotes.trim() !== "");
      ...
      if (isCompletedWithPayload) {
        ...
        res = await fetch(
          `${API_BASE_URL}/volunteers/tasks/${taskId}/change-status`,
          {
            method: "POST",
            headers: {
              Accept: "application/json",
      ...
      setSubmitError(
        err instanceof Error ? err.message : t("taskDetail.statusUpdateFailed"),
      );
    } finally {
      setSubmitting(false);
    }
      ...
      <div className="mx-auto max-w-4xl">
        ...
        >
              ...
              <div>
                ...
                <p className="text-xl font-bold">
                  {task.due_date_formatted ||
                    new Date(task.deadline).toLocaleDateString("bn-BD")}
                </p>
              </div>
            </div>
          ...
          <div className="flex items-center gap-3 flex-wrap">
            <span
              className={`px-4 py-2 rounded-xl font-bold text-sm border-2 ${getPriorityColor(task.priority)}`}
            >
              {t("tasks.priority")} {getPriorityLabel(task.priority)}
            </span>
            <span
              className={`px-4 py-2 rounded-xl font-bold text-sm border-2 ${getStatusColor(task.status)}`}
            >
