  ...
  useEffect(() => {
    const fetchPrograms = async () => {
      try {
        ...
        const response = await syncedFetch(`${apiBaseUrl}/programs`, {
        ...
        const mappedPrograms: Program[] = programsData.map(
          (program: any, index: number) => {
            ...
            if (program.image) {
              ...
              if (image.startsWith("http://") || image.startsWith("https://")) {
                ...
                imageUrl =
                  image.startsWith("storage/") || image.startsWith("/storage/")
                    ? `https://admin.aminul-haque.com/${image.replace(/^\//, "")}`
                    : image;
              }
            }
            const description =
              program.bangla_description ||
              program.description ||
              program.details ||
              "";
            return {
              ...
              color:
                program.color || defaultColors[index % defaultColors.length],
              image: imageUrl,
            };
          },
        );
          ...
          {!error &&
            programs.map((program, idx) => {
              ...
              return (
                ...
                >
                  <div
                    className={`absolute inset-0 rounded-3xl blur-2xl opacity-20 group-hover:opacity-30 transition-all`}
                  ></div>
                    ...
                    >
                      ...
                      <div className="w-full md:w-1/2">
                        <div className={`${isEven ? "md:pr-4" : "md:pl-4"}`}>
                          ...
                          {program.tagline && (
                            <p
                              className={`text-lg md:text-xl font-bold bg-gradient-to-r ${program.color} bg-clip-text text-transparent mb-4`}
                            >
                              {program.tagline}
                            </p>
                          )}
